...
DRV89xxMotor::DRV89xxMotor(byte hb1, byte hb2, byte pwm_channel, byte reverse_delay, bool free_wheeling) : _free_wheeling(free_wheeling), _reverse_delay(reverse_delay), _pwm_channel(pwm_channel)
{
...
void DRV89xxMotor::populateHalfbridgeOffsets(byte offset, byte half_bridge)
{
    ...
    _half_bridge[offset].fw_ctrl_register = (byte)DRV89xxRegister::FW_CTRL_1 + (half_bridge - 1) / 8;      // FW_CTRL_2 if hb 9+
...
void DRV89xxMotor::applyConfig(byte *settings)
{
    ...
    {
        switch (_direction)
        {       // this section is verbose verbose, but much clearer than the more line-efficient logic would be
        case 1: // forward
            ...
            {                                                          // brake for _reverse_delay upon direction reversing
                setBridgeLowsideDisablePWM(settings, _half_bridge[0]); // hb1 LOW, no PWM
    ...
    { // motor is disabled, set gates to ZZ (free spinning), disable PWM channel for bridges
...
void DRV89xxMotor::setBridgeLowsideDisablePWM(byte *settings, DRV89xxHalfBridge &bridge)
{
    ...
    Serial.println(bridge.id);
    Serial.print("Brake HBridge L: 0x");
    Serial.print(bridge.enable_register, HEX);
    Serial.print(" set bit ");
    Serial.println(bridge.bitshift_2);
    Serial.print("Brake HBridge H: 0x");
    Serial.print(bridge.enable_register, HEX);
    Serial.print(" clear bit ");
    Serial.println(bridge.bitshift_2 + HIGH_SIDE);
    Serial.print("Brake HBridge PWM: 0x");
    Serial.print(bridge.pwm_ctrl_register, HEX);
    Serial.print(" clear bit ");
    Serial.println(bridge.bitshift_1);
#endif
...
void DRV89xxMotor::setBridgeHSPWM(byte *settings, DRV89xxHalfBridge &bridge)
{
    ...
    Serial.print(" set bit ");
    Serial.println(bridge.bitshift_1);
    Serial.print("Setting PWM channel: 0x");
    Serial.print(bridge.pwm_map_register, HEX);
    Serial.print(" bytes at ");
    Serial.print(bridge.bitshift_2);
    Serial.print(",");
    Serial.print(bridge.bitshift_2 + 1);
    Serial.print(": ");
    Serial.println(_pwm_channel);
#endif
    ...
    if (_free_wheeling)
    {
        BIT_SET(settings[bridge.fw_ctrl_register], bridge.bitshift_1); // enable PWM on this half bridge
    }
    else
    {
        BIT_CLEAR(settings[bridge.fw_ctrl_register], bridge.bitshift_1); // enable PWM on this half bridge
    }
}
